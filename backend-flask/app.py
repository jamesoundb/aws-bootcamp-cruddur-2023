...
# CloudWatch Logs modules and imports
import watchtower
import logging
from time import strftime

# Xray modules and imports
from aws_xray_sdk.core import xray_recorder
from aws_xray_sdk.ext.flask.middleware import XRayMiddleware

# HoneyComb modules imports
...
# cw_handler = watchtower.CloudWatchLogHandler(log_group='jameswurbel-cruddur')
...
def data_message_groups():
  ...
  try:
    claims = cognito_token_verify.verify(access_token)
    # authenticated request
    app.logger.debug('authenticated')
    app.logger.debug(claims["sub"])
    user_id = claims['sub']
    model = MessageGroups.run(cognito_user_id=user_id)
...
def data_messages(message_group_uuid):
  ...
  try:
    ...
    app.logger.debug(claims["sub"])
    user_id = claims['sub']
    model = Messages.run(cognito_user_id=user_id, message_group_uuid=message_group_uuid)
    if model['errors'] is not None:
      return model['errors'], 422
    else:
      return model['data'], 200
...
def data_create_message():
  ...
  try:
    ...
    if message_group_uuid == None:
      ...
      model = CreateMessage.run(mode="create",
        message=message,
